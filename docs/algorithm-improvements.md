...
## 第二十二版：全 ULD 顶层收益重排与实例编号归一

动机：4 个已用 ULD 装 42 箱的现场数据中，Q5-001 顶层的 BOX-E
以 `120×80` 横跨两个 BOX-B。把它旋转为 `80×120` 后，旁边还能再放
一个同型箱，人工布局通过边界、碰撞和支撑率全量校验。旧局部重排只按
装载率从低到高检查 3 个 ULD；Q5-001 恰好是 4 个 ULD 中装载率最高的，
所以已有的“拆顶层再补空”能力从未在它上面执行。

同一数据还暴露了重排后的实例编号复用：箱子从多个 ULD 腾空再装入时，
实例号按当前剩余数量重新推导，最终可能在不同 ULD 中同时出现
`BOX-E-019`，导致页面选择和箱体追踪不再唯一。

做法：

```text
单 ULD 顶层重排逐个评估所有已使用 ULD，不再用低装载率排名代替实际收益
每轮选择全局评分提升最大的候选，最多接受 LOCAL_REARRANGE_MAX_PASSES 次改进
完整腾空和双 ULD 联合重装仍沿用原 Top-K 目标筛选，控制高成本搜索的耗时
多 ULD 最终结果生成前，按容器顺序和放置顺序对每个箱型连续重编号
结果校验增加跨 ULD 重复 instance_id 检查，防止编号回归静默进入输出
```

对应测试：

```text
test_top_and_partitioned_layer_rearrange_improve_field_layout_and_keep_ids_unique
```

效果：

```text
现场数据从 39 箱提升到 40 箱，未装 BOX-E 从 1 个降为 0
Q5-001 自动生成两个 80×120×121 的顶层 BOX-E，装载率 66.30% → 72.27%
全部放置通过合法性校验，最终 40 个 instance_id 全局唯一
单个确定性高装载率轮次本机约 0.66 秒，未观察到该用例耗时回退
```

## 第二十三版：左右分块混排层与定向重装

动机：第二十二版修复后，同一现场数据的 Q5-002 底层仍把 6 个
`120×80×121` BOX-E 排成 `2×3` 整行网格。人工方案把底层沿 x 分区：
左侧一列保持 `120×80` 放 3 个，右侧两列旋转成 `80×120` 放 4 个，
底层可从 6 个提升到 7 个。把一个上层 BOX-E 下移后，上层还能增加一个
高箱，完整方案通过截面、碰撞和支撑率校验。

诊断发现原层构建只枚举沿 y 叠加的贯穿全长行，每一行使用一种朝向；
它能表达 `6+5` 这类横向行混排，但不能表达左、右矩形块分别使用不同朝向。
普通逐箱候选可以表达该布局，不过在完整多 ULD 搜索中需要多步才能形成，
会被一次推进 6 箱的整层分支提前挤出 Beam。

做法：

```text
_x_partition_layer_layout 枚举沿 x 的所有左右切分位置
左右块分别用一种朝向铺规则网格，两个方向都尝试作为左块
仅当分块布局箱数严格大于原整行布局时才视为改进候选
主 Beam 继续使用原整行层，避免高数量小箱在早期改变全局箱型竞争
高装载率收尾逐个腾空 ULD，以更密的分块层为种子重新装填
种子后的剩余箱继续走 Beam、补空和全量合法性校验，全局评分更高才接受
```

对应测试：

```text
test_partitioned_layer_layout_packs_seven_rotatable_boxes
test_top_and_partitioned_layer_rearrange_improve_field_layout_and_keep_ids_unique
```
...
```text
Q5-002 底层 BOX-E 从 6 个提升到 7 个，布局为左侧 1×3 + 右侧 2×2
现场数据从 40 箱提升到 41 箱，仅剩 BOX-D 1 个未装
Q5-002 装载率 61.57% → 70.39%，总体积利用率 66.20% → 68.47%
最终方案装入体积更大的 BOX-A 和两个 BOX-B，比仅补一个 BOX-B 的人工方案体积更高
全部放置和跨 ULD 实例编号校验通过，确定性轮次本机约 0.78 秒
```

## 第二十四版：CP-SAT 单 ULD 局部优化后端

动机：前二十三版仍依赖人为识别“顶层旋转”“左右分块”等搜索空间缺口。
这些通用启发式能解决一类问题，但无法给小规模局部重排提供系统化组合搜索。
高装载率模式需要一个与 Beam 搜索机制不同的后端，联合决定箱子取舍、朝向和
三维坐标，并且只在结果更好时替换启发式方案。

做法：

```text
引入 OR-Tools 9.15 CP-SAT，并保留依赖不可用时的启发式回退
只在 high_utilization 收尾阶段优化装载率最低的一个已用 ULD
局部物品集合 = 该 ULD 当前箱子 + 能进入该 ULD 的全局未装箱子
最多建模 16 箱，单次 CP-SAT 时间上限 0.35 秒，超限直接保留启发式方案
每个箱子用 presence、orientation、x/y/z 变量联合决定是否装入和摆放方式
凸截面每条边转成四角点线性约束；任意两箱用六方向析取约束避免重叠
非地面箱必须选择一个下方主支撑箱，覆盖率按 (r,1)、(sqrt(r),sqrt(r))、(1,r) 三种充分条件建模
CP-SAT 解仍通过 validate_profile_packing 独立复查，非法方案不会进入最终结果
最终候选必须严格提高原全局评分，不能以 CP-SAT 名义接受数量或体积回退
```

支撑模型目前是保守子集：可以表达由一个主支撑面满足阈值的放置，多个下方箱
共同拼出支撑面积但任一单箱都不足的方案仍交给原启发式处理。它因此是局部组合
优化后端，不是完整业务约束下的全局最优证明。

对应测试：

```text
test_optimizer_rearranges_container_and_loads_remaining_box
test_optimizer_models_supported_stacking_inside_sloped_profile
test_optimizer_does_not_move_required_box_into_disallowed_container
```
...
矩形用例可自动重排已装箱并装入剩余箱
Q5 斜边截面可生成合法的两层支撑布局
required_container_types 在局部优化中继续作为硬约束
现场数据保持 41 箱 / 68.47%，确定性轮次约 0.78 秒 → 1.19 秒
完整 8 轮高装载率入口约 2.53 秒 → 4.37 秒，仍在分钟级预算内
无 OR-Tools 环境自动回退到原启发式路径，标准库回退测试可独立运行
桌面构建流程安装项目依赖，确保发布产物包含 CP-SAT 后端
```

## 第二十五版：独立搜索模式的预算与评分修正

fast、balanced 和 high_utilization 保持三套独立搜索，不复用或回退到其它模式的结果。
现场倒挂来自大规模档的 balanced 基础 Beam 只有 3：第三条短期高分路径在第 11 步
产生大量候选，把两条长期可行路径裁掉。balanced 的大规模 Beam 下限提高到 4，并继续
执行两轮独立 GRASP；该现场数据的 balanced 最优轮由 260 箱提高到 264 箱。

同时约束 fast 的各搜索上限不超过基础档位，避免极小 Beam 配置下 fast 反而比 balanced
更宽。三种模式仍是启发式算法，独立搜索不承诺数学上的严格单调，但不会用低档结果冒充
高档结果。

完整结果择优也统一到目标定义：人工指定 ULD 的未装数保持最高优先级；其后
...
+ 多 ULD 最终实例统一编号：每个箱型按最终放置顺序连续编号，跨 ULD 全局唯一
+ 左右分块混排层：高装载率收尾用严格优于整行布局的 x 分区层定向重装单个 ULD
+ CP-SAT 局部优化：高装载率收尾对小规模单 ULD 联合优化箱子取舍、朝向和坐标
```
...
CP-SAT 扩展：候选多个 ULD 联合重装，并精确表达多支撑面面积求和
...
如果后续需要数学意义上的严格最优解，应继续扩展 CP-SAT 的支撑模型和多 ULD 联合模型，并输出求解上界与最优差距。
